...
authTest.describe("Filtros da Listagem de Solicitações", () => {
  ...
  authTest("S3 — filtro por médico reduz os cards exibidos", async ({ authenticatedPage: page }) => {
    // Mock com resposta filtrada quando doctor_id está presente
    await page.route(`${API_BASE}/surgery-requests**`, async (route) => {
      const url = new URL(route.request().url());
      const doctorId = url.searchParams.get("doctor_id") || url.searchParams.get("manager_id");
    ...
    const doctorFilter = page
      .locator('[data-testid="filter-doctor"], select[name*="doctor"], [placeholder*="médico"], [placeholder*="doctor"]')
      .first();
    ...
    if (await doctorFilter.isVisible({ timeout: 5_000 })) {
      ...
      if (tagName === "select") {
        ...
        const option = page.getByRole("option", { name: /Dr. Admin|Admin Teste/i }).first();
      ...
      console.log("Filtro de médico não localizado — revisar seletores para esta versão da UI");
    }
  });

  authTest("S4 — filtro por convênio reduz os cards exibidos", async ({ authenticatedPage: page }) => {
    await page.route(`${API_BASE}/surgery-requests**`, async (route) => {
      const url = new URL(route.request().url());
      ...
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify(hpId ? [mockSurgeryRequestListResponse[0]] : mockSurgeryRequestListResponse),
      });
    });
    ...
    const hpFilter = page
      .locator('[data-testid="filter-health-plan"], select[name*="health"], [placeholder*="convênio"]')
      .first();

    if (await hpFilter.isVisible({ timeout: 5_000 })) {
      const tagName = await hpFilter.evaluate((el) => el.tagName.toLowerCase());
      if (tagName === "select") {
  ...
  authTest("S3/S4 — limpar filtro restaura todas as solicitações", async ({ authenticatedPage: page }) => {
    await page.route(`${API_BASE}/surgery-requests**`, async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
    ...
    const clearButton = page.getByRole("button", { name: /limpar|resetar|clear/i }).first();
    if (await clearButton.isVisible({ timeout: 3_000 })) {
      ...
      await expect(page.locator("text=João Silva").first()).toBeVisible({ timeout: 5_000 });
      await expect(page.locator("text=Maria Souza").first()).toBeVisible({ timeout: 5_000 });
    }
  });

  authTest("busca por texto filtra solicitações client-side", async ({ authenticatedPage: page }) => {
    await page.route(`${API_BASE}/surgery-requests**`, async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
