...
export default function FornecedoresPage() {
  ...
  const selectedItems = useMemo(() => {
    return Object.keys(rowSelection)
      ...
      .map((key) => filteredSuppliers[parseInt(key)])
      .filter(Boolean);
  }, [rowSelection, filteredSuppliers]);
  ...
  const columns: ColumnDef<Supplier>[] = [
    ...(podeAdministrar ? [createSelectColumn<Supplier>()] : []),
    {
      accessorKey: "name",
      header: "Nome",
      size: 250,
      cell: ({ row }) => (
        <div
          ...
          onClick={() =>
            router.push(`/colaboradores/fornecedor/${row.original.id}`)
          }
        >
          <div
    ...
    ...(podeAdministrar
      ? [
          createDeleteActionColumn<Supplier>(
  ...
  return (
    <PageContainer className="border-gray-200">
      <div className="flex-none flex items-center gap-2 px-4 lg:px-8 py-3 border-b border-gray-200">
        <h1 className="ds-page-title">Fornecedores</h1>
      </div>
