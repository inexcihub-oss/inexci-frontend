...
export default function FabricantesPage() {
  ...
  // excluir continua sendo ato do admin, porque apaga um fabricante que OPMEs
  // já referenciam.
  ...
  const filteredManufacturers = useMemo(() => {
    if (!debouncedSearchTerm) return manufacturers;
  ...
  }, [manufacturers, debouncedSearchTerm]);

  const selectedItems = useMemo(() => {
    return Object.keys(rowSelection)
      .filter((key) => (rowSelection as Record<string, boolean>)[key])
      .map((key) => filteredManufacturers[parseInt(key, 10)])
      .filter(Boolean);
  }, [rowSelection, filteredManufacturers]);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await manufacturerService.delete(deleteModal.id);
      setManufacturers((prev) => prev.filter((m) => m.id !== deleteModal.id));
      setDeleteModal({ open: false, id: null, name: null, loading: false });
    } catch (error) {
      logger.error("Erro ao excluir fabricante:", error);
  ...
  const columns: ColumnDef<Manufacturer>[] = [
    ...(podeExcluir ? [createSelectColumn<Manufacturer>()] : []),
    {
      ...
      cell: ({ row }) => (
        <div
          ...
          onClick={() =>
            router.push(`/colaboradores/fabricante/${row.original.id}`)
          }
        >
          <div
    ...
    {
      accessorKey: "anvisaRegistration",
      ...
      cell: ({ row }) => (
        <span
          className="text-xs text-black"
          title={row.original.anvisaRegistration || "-"}
        >
          {row.original.anvisaRegistration || "-"}
        </span>
      ),
    },
    {
    ...
    ...(podeExcluir
      ? [
          createDeleteActionColumn<Manufacturer>(
  ...
  return (
    <PageContainer className="border-gray-200">
      <div className="flex-none flex items-center gap-2 px-4 lg:px-8 py-3 border-b border-gray-200">
        <h1 className="ds-page-title">Fabricantes</h1>
      </div>
