...
export default function ClinicasPage() {
  ...
  const selectedItems = useMemo(() => {
    return Object.keys(rowSelection)
      ...
      .map((key) => filteredClinics[parseInt(key)])
      .filter(Boolean);
  }, [rowSelection, filteredClinics]);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await clinicService.delete(deleteModal.id);
      queryClient.setQueryData<Clinic[]>(CLINICS_QUERY_KEY, (prev = []) =>
  ...
  const handleBulkDeleteConfirm = async () => {
    ...
    try {
      await clinicService.deleteMany(ids);
      queryClient.setQueryData<Clinic[]>(CLINICS_QUERY_KEY, (prev = []) =>
  ...
  const columns: ColumnDef<Clinic>[] = [
    ...(podeExcluir ? [createSelectColumn<Clinic>()] : []),
    {
      accessorKey: "name",
      header: "Nome",
      size: 250,
      cell: ({ row }) => (
        <div
          ...
          onClick={() => router.push(`/clinicas/${row.original.id}`)}
        >
          <div
    ...
    ...(podeExcluir
      ? [
          createDeleteActionColumn<Clinic>(
  ...
  return (
    <PageContainer className="border-gray-200">
      <div className="flex-none flex items-center gap-2 px-4 lg:px-8 py-3 border-b border-gray-200">
        <h1 className="ds-page-title">Clínicas</h1>
      </div>
      ...
      <ConfirmDeleteModal
        ...
        title={`Excluir ${selectedItems.length} clínica${selectedItems.length !== 1 ? "s" : ""}`}
        description={`Tem certeza que deseja excluir ${selectedItems.length} clínica${selectedItems.length !== 1 ? "s" : ""} selecionada${selectedItems.length !== 1 ? "s" : ""}? Esta ação não pode ser desfeita.`}
