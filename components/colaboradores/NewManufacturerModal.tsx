...
const FIELD_LABELS: Record<string, string> = {
  name: "Nome",
  cnpj: "CNPJ",
  anvisaRegistration: "Registro ANVISA",
  phone: "Telefone",
  email: "E-mail",
...
export function NewManufacturerModal({
  isOpen,
  onClose,
  onSuccess,
    ...
    async (data) => {
      ...
      try {
        const payload: CreateManufacturerPayload = {
          ...
          anvisaRegistration: data.anvisaRegistration || undefined,
        ...
        await manufacturerService.create(payload);
        onSuccess();
        form.reset();
        onClose();
      } catch (err) {
        ...
        setError(
          Array.isArray(msg)
            ? msg.join(", ")
            : msg || "Erro ao criar fabricante. Tente novamente.",
        );
      } finally {
        setLoading(false);
      }
          ...
          <div className="px-4 py-4 md:px-6 md:py-6 flex flex-col gap-3 md:gap-5 overflow-y-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Input
                ...
                {...form.getFieldProps("anvisaRegistration")}
              />
              <Input
                label="Telefone"
                type="tel"
            ...
            <Input
              label="E-mail"
              type="email"
              placeholder="fabricante@mail.com"
